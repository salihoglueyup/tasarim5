...
export default function TeklifAl() {
  ...
  const steps = [
    {
      id: 1,
      question: t('quote_step_1_q'),
      placeholder: t('quote_step_1_ph'),
      type: "text",
    },
    {
      id: 2,
      question: t('quote_step_2_q'),
      placeholder: t('quote_step_2_ph'),
      type: "number",
    },
    {
      id: 3,
      question: t('quote_step_3_q'),
      options: [
        t('quote_step_3_opt_1'), 
        t('quote_step_3_opt_2'), 
        t('quote_step_3_opt_3'), 
        t('quote_step_3_opt_4')
      ],
      type: "select",
    },
    {
      id: 4,
      question: t('quote_step_4_q'),
      placeholder: t('quote_step_4_ph'),
      type: "textarea",
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#111111] flex flex-col justify-center px-[var(--spacing-gutter)] relative overflow-hidden font-sans">
      ...
      <Link href="/" className="absolute top-12 left-12 flex items-center gap-2 text-white/50 hover:text-white transition-colors z-50">
        ...
        <span className="text-sm font-medium tracking-widest uppercase">{t('quote_back_btn')}</span>
      </Link>
          ...
          {!isCompleted ? (
            ...
            >
              ...
              <div className="flex items-center gap-6 mt-16">
                ...
                >
                  {currentStep === steps.length - 1 ? t('quote_submit_btn') : t('quote_next_btn')}
                  <span className="material-symbols-outlined text-base">done</span>
                </button>
                
                {!answers[currentStep] && currentStepData.type !== 'select' && (
                  <span className="text-white/30 text-sm">{t('quote_hint')}</span>
                )}
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl font-bold text-white mb-8 tracking-tight">{t('quote_success_title')}</h1>
