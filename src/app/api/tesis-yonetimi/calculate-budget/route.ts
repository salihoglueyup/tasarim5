...
export function calculateFacilityBudget(
  ...
  // Ölçek ekonomisi indirimi: Birim sayısı arttıkça sabit giderler düşer
  ...
  // Alo Yönetim Tasarruf Modeli (%30 tasarruf)
  ...
  return {
    ...
    breakdown: {
      security: {
        ...
        percentage: 35,
        description: '5188 Lisanslı Özel Güvenlik, CCTV İzleme ve Devriye Personeli',
      },
      cleaning: {
        amount: cleaningAmount,
        percentage: 25,
        description: 'TSE HYB Standartlarında Kat Temizliği, Kimyasal ve Biyosidal İlaçlama',
      },
      technicalMaintenance: {
        amount: technicalAmount,
        percentage: 20,
        description: 'ISO 41001 Periyodik Asansör, Jeneratör, Kompanzasyon ve Yangın Sistemleri',
      },
      commonEnergyAndUtilities: {
        amount: energyAmount,
        percentage: 12,
        description: 'Ortak Alan Aydınlatma, Hidrofor Enerjisi ve Tesis Ortak Su/Doğalgaz Payı',
      },
      kmkLegalEmergencyReserve: {
        amount: reserveAmount,
        percentage: 8,
        description: '634 Sayılı KMK Madde 20 Kapsamında Olağanüstü Onarım & Amortisman Fonu',
      },
    },
    savingsWithAloYonetim: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const units = typeof body.units === 'number' ? body.units : parseInt(body.units || '30', 10);
    const facilityType = (body.facilityType || body.type || 'site') as FacilityType;
    const district = body.district || 'kadikoy';
    const floorAreaM2 = body.floorAreaM2 ? parseInt(body.floorAreaM2, 10) : undefined;
