      ...
      <section className="py-20 px-[var(--spacing-gutter)] max-w-[var(--spacing-container-max)] mx-auto">
        <div className="bg-[var(--color-surface)] border border-[var(--color-outline)]/60 p-10 md:p-16 rounded-[3rem] shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 flex flex-col gap-6">
            <span className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-widest bg-slate-900/10 dark:bg-white/10 px-4 py-1.5 rounded-full w-fit">
          ...
          <div className="lg:col-span-5 bg-gradient-to-br from-slate-900 to-[#1e293b] text-white p-10 rounded-[2.5rem] flex flex-col gap-6">
            <div className="text-4xl font-extrabold text-slate-300">{t('about_manifest_stat')}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {leaders.map((l, i) => (
            <div key={i} className="bg-[var(--color-surface)] border border-[var(--color-outline)]/60 p-8 rounded-[2.5rem] flex flex-col gap-4 text-center items-center shadow-sm hover:-translate-y-2 transition-transform duration-300">
              <Image src={l.avatar} alt={l.name} width={96} height={96} className="w-24 h-24 rounded-full object-cover border-2 border-slate-800 dark:border-white/20" />
              <div>
                <h3 className="text-xl font-bold text-[var(--color-primary)]">{l.name}</h3>
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">{l.title}</p>
              </div>
      ...
      <section className="py-20 px-[var(--spacing-gutter)] max-w-[var(--spacing-container-max)] mx-auto">
        ...
        <div className="relative max-w-4xl mx-auto" ref={containerRef}>
          ...
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-1 bg-slate-200 dark:bg-slate-800 -translate-x-1/2 rounded-full hidden md:block">
            <motion.div 
              className="w-full bg-slate-900 dark:bg-white rounded-full"
            ...
            {timeline.map((t, i) => (
              <div key={i} className={`flex flex-col md:flex-row items-center justify-between gap-8 ${i % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                <div className="flex-1 w-full text-left md:text-right">
                  {i % 2 === 0 && (
                    <div className="hidden md:block">
                      <div className="text-5xl font-extrabold text-slate-900/10 dark:text-white/10">{t.year}</div>
                ...
                <div className="w-16 h-16 rounded-full bg-[var(--color-surface)] border-4 border-[var(--color-outline)] shadow-xl flex items-center justify-center z-10 hidden md:flex shrink-0">
                  <div className="w-6 h-6 rounded-full bg-slate-900 dark:bg-white animate-pulse"></div>
                </div>

                <div className="flex-1 w-full text-left md:text-left bg-[var(--color-surface)] p-8 rounded-3xl border border-[var(--color-outline)] shadow-sm relative overflow-hidden group hover:border-slate-500/50 transition-colors duration-300">
                  ...
                  <div className="relative z-10">
                    <span className="inline-block md:hidden text-2xl font-black text-slate-900 dark:text-white mb-2">{t.year}</span>
