...
export default function SozlukClient({ lang = "tr" }: { lang?: string }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface)] font-sans text-slate-800 dark:text-slate-200">
        ...
        <div className="max-w-7xl mx-auto px-[var(--spacing-gutter)] relative z-10">
          <div className="flex flex-col items-center text-center gap-5 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full mt-4">
              ...
              <div className="p-3.5 rounded-2xl bg-white/[0.06] backdrop-blur-md border border-white/10 shadow-sm flex flex-col items-center text-center">
                <span className="text-xl md:text-2xl font-black text-slate-100">KMK 634</span>
                <span className="text-[11px] font-semibold text-slate-400 mt-0.5">Mevzuat Uyumlu</span>
              </div>
              <div className="p-3.5 rounded-2xl bg-white/[0.06] backdrop-blur-md border border-white/10 shadow-sm flex flex-col items-center text-center">
                <span className="text-xl md:text-2xl font-black text-slate-100">5188</span>
                <span className="text-[11px] font-semibold text-slate-400 mt-0.5">Özel Güvenlik</span>
              </div>
              <div className="p-3.5 rounded-2xl bg-white/[0.06] backdrop-blur-md border border-white/10 shadow-sm flex flex-col items-center text-center">
                <span className="text-xl md:text-2xl font-black text-slate-100">ISO 41001</span>
      ...
      {/* 2. Google AI Overviews & Ground-Truth Tanımlar */}
      ...
      {/* 3. Ana Sözlük Modülü (DefinedTermSetSeo) */}
      ...
      {/* 4. Google Position Zero / AI Overviews Master Özet Rehberi */}
      <section className="py-16 md:py-20 border-t border-[var(--color-outline)]/60 bg-[var(--color-surface-variant)]/30">
        <div className="max-w-7xl mx-auto px-[var(--spacing-gutter)]">
          
          <div className="flex flex-col gap-4 text-center max-w-3xl mx-auto mb-12">
            <span className="text-xs font-extrabold uppercase tracking-widest text-[var(--color-primary)] bg-[var(--color-surface-variant)] px-3.5 py-1.5 rounded-full w-fit mx-auto border border-[var(--color-outline)]/80">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-[var(--color-surface)] border border-[var(--color-outline)]/80 rounded-3xl p-6 flex flex-col justify-between shadow-xs hover:border-[var(--color-primary)]/40 hover:shadow-md transition-all">
              <div className="space-y-3">
                <div className="w-10 h-10 rounded-2xl bg-[var(--color-surface-variant)] text-[var(--color-primary)] border border-[var(--color-outline)]/60 flex items-center justify-center">
                  <WalletSvgIcon className="w-5 h-5 text-[var(--color-primary)]" />
                </div>
      ...
      {/* 5. KMK 634 Temel Kanun Maddeleri Referans Tablosu */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {KMK_LAW_INDEX.map((law) => (
              <div 
                ...
                className="bg-[var(--color-surface)] border border-[var(--color-outline)]/80 rounded-3xl p-5.5 flex flex-col justify-between shadow-xs hover:border-[var(--color-primary)]/40 hover:shadow-md transition-all"
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-[var(--color-primary)] bg-[var(--color-surface-variant)] px-2.5 py-1 rounded-lg border border-[var(--color-outline)]/80">
                      KMK Madde {law.articleNumber}
                    </span>
                    <span className="text-[10px] font-mono text-[var(--color-secondary)] uppercase">{law.category}</span>
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-[var(--color-outline)]/40 text-[11px] text-[var(--color-secondary)]">
                  <strong className="text-[var(--color-primary)] font-semibold">Uygulama:</strong> {law.practicalApplication}
                </div>
              </div>
            ))}
          </div>
      ...
      {/* 6. Sıkça Sorulan Sorular Akordeon */}
      <section className="py-16 md:py-20 border-t border-[var(--color-outline)]/60 bg-[var(--color-surface-variant)]/30">
        <div className="max-w-4xl mx-auto px-[var(--spacing-gutter)]">
          ...
          <div className="space-y-3">
            {faqItems.map((item, index) => (
              <div 
                key={index}
                className="bg-[var(--color-surface)] border border-[var(--color-outline)]/80 rounded-2xl overflow-hidden transition-all shadow-xs"
              >
                <button
                  type="button"
      ...
      {/* 7. B2B Keşif & Hukuki Danışmanlık CTA Kartı */}
      <section className="py-16 md:py-24 px-[var(--spacing-gutter)] max-w-5xl mx-auto">
        <div className="bg-[var(--color-surface)] text-[var(--color-primary)] rounded-[2.5rem] p-8 md:p-14 text-center flex flex-col items-center gap-6 shadow-xs relative overflow-hidden border border-[var(--color-outline)]/80">
          ...
          <p className="text-xs md:text-sm text-[var(--color-secondary)] font-normal max-w-xl leading-relaxed">
