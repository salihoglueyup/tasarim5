  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Kategoriler</h1>
          <p className="text-sm text-slate-500 dark:text-gray-400 mt-1">Blog kategorilerini ve alt kategorileri yönetin.</p>
        </div>
        <Link 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm divide-y divide-slate-100 dark:divide-white/5">
              {categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                  <td className="p-4">
                    <div className="font-medium text-slate-900 dark:text-gray-200 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{cat.name}</div>
                    <div className="text-xs text-slate-500 dark:text-gray-500 mt-1 font-mono">/{cat.slug}</div>
                  </td>
                  <td className="p-4">
                    {cat.parent ? (
                      ...
                      <span className="text-slate-400 dark:text-gray-500 italic text-xs">- Ana Kategori -</span>
                    )}
                  </td>
