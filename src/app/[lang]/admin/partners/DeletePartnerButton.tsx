...
export default function DeletePartnerButton({ id, lang, title }: { id: string, lang: string, title: string }) {
  const [isDeleting, setIsDeleting] = useState(false);
  const router = useRouter();

  const handleDelete = async () => {
    if (confirm(`"${title}" adlı iş ortağını silmek istediğinize emin misiniz?`)) {
      setIsDeleting(true);
