  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Blog Yazıları</h1>
          <p className="text-sm text-gray-400 mt-1">Sitedeki tüm blog yazılarını buradan yönetebilirsiniz.</p>
        </div>
        <Link 
            ...
            <tbody className="text-sm divide-y divide-white/5">
              {posts.map((post) => (
                <tr key={post.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="p-4">
                    <div className="font-medium text-gray-200 group-hover:text-brand-400 transition-colors">{post.title}</div>
                  ...
                  <td className="p-4 text-gray-400 whitespace-nowrap">{post.author.name}</td>
                  <td className="p-4 whitespace-nowrap">
                    {post.published ? (
                      <span className="inline-flex items-center px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs">
                        Yayında
                      </span>
                    ) : (
                      <span className="inline-flex items-center px-2 py-1 rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs">
                        Taslak
                      </span>
                    )}
                  </td>
