    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 backdrop-blur-sm space-y-6">
        ...
        <div className="flex items-center space-x-6">
          <div className="w-24 h-24 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden flex-shrink-0">
            {formData.avatar ? (
              <img src={formData.avatar} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
              <span className="text-3xl text-white/50">{formData.name ? formData.name.charAt(0).toUpperCase() : '👤'}</span>
            )}
          </div>
          
          <div className="flex-1 space-y-3">
            <label className="block text-sm font-medium text-gray-300">Profil Fotoğrafı (Avatar)</label>
            <label className="inline-flex items-center px-4 py-2 bg-white/5 border border-white/10 rounded-lg cursor-pointer hover:bg-white/10 transition-colors text-sm">
            ...
            <div className="flex items-center space-x-2 w-full mt-2">
              <span className="text-xs text-gray-500 whitespace-nowrap">veya URL girin:</span>
              <input
                type="text"
        ...
        <div className="pt-4 border-t border-white/5">
          <label className="block text-sm font-medium text-gray-300 mb-2">Yazar / Ekip Adı</label>
          <input
            type="text"
            required
            value={formData.name}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Biyografi (Hakkında)</label>
          <textarea
            rows={4}
            value={formData.bio}
        ...
        <div className="pt-4 border-t border-white/10">
          <button
            type="submit"
            disabled={loading}
            className="w-full sm:w-auto bg-gradient-to-r from-brand-600 to-brand-500 text-white font-medium rounded-xl px-8 py-3 hover:from-brand-500 hover:to-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all disabled:opacity-50"
          >
