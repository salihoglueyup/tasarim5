...
const ProjectCard = ({ project, isLarge, lang, router }: { project: Project, isLarge: boolean, lang: string, router: any }) => {
  ...
  // Mouse konumunu takip eden değerler
  const x = useMotionValue(0);
  const y = useMotionValue(0);

  // Yumuşak geçiş için yay (spring) ayarları
  const mouseXSpring = useSpring(x, { stiffness: 150, damping: 15 });
  const mouseYSpring = useSpring(y, { stiffness: 150, damping: 15 });

  // Transform eşleştirmesi (-7 ile +7 derece arası eğim)
  ...
  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    ...
    // Değerleri -0.5 ile +0.5 arasına normalize et
  ...
  return (
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-[2rem] bg-slate-900 border border-[var(--color-outline)]/40 shadow-sm hover:shadow-2xl transition-shadow duration-500 cursor-pointer ${
    ...
    >
      ...
      {/* Shine (Parlama) Efekti - Sağ alt çaprazdan geçer */}
...
export default function ReferencesClient({ 
  ...
  return (
    <>
      ...
      <section className="py-24 px-[var(--spacing-gutter)] max-w-[var(--spacing-container-max)] mx-auto" style={{ perspective: "1000px" }}>
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[350px] md:auto-rows-[400px]">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, i) => {
              // Asimetrik bento grid: Bazı kartları daha geniş tutar
        ...
        {filteredProjects.length === 0 && (
          <div className="text-center py-20 text-[var(--color-secondary)]">
            <span className="material-symbols-outlined text-4xl mb-4 opacity-50 block">search_off</span>
