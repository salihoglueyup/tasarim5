...
/**
 * Faz 57: PwaInstallPrompt optimizasyonu:
 * - Framer Motion kaldırılmış, saf GPU CSS geçişlerine geçilmiştir.
 * - Kullanıcı deneyimini ve ilk oturum odaklanmasını bölmemek için bildirim
 *   yalnızca kullanıcı 30 saniye sitede kaldıktan sonra (deferred) gösterilir.
 */
export function PwaInstallPrompt() {
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      // Faz 57: Kullanıcı deneyimini ve dönüşüm hunisini bölmemek için 30 saniye gecikmeyle göster
    ...
    >
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
            >
              Daha Sonra
            </button>
          </div>
        ...
        <button
          type="button"
          onClick={handleDismiss}
          className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 -mr-1 -mt-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          aria-label="Kapat"
        >
          <X className="w-4 h-4" />
        </button>
