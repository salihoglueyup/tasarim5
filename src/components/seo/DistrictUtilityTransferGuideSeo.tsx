  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <h2 className="text-xl sm:text-3xl font-extrabold text-white tracking-tight">
            İSKİ, BEDAŞ / AYEDAŞ & İGDAŞ <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400">Ortak Sayaç Devri ve Mesken Tarifesi Kılavuzu</span>
          </h2>
        ...
        <span className="text-xs text-slate-400">48 Saatte Hızlı Bürokratik Devir</span>
      </div>

      {/* Provider Selector Tabs */}
      <div className="flex flex-wrap gap-2 p-1.5 bg-slate-800/60 rounded-2xl border border-slate-700/80 mb-8">
      ...
      <div className="space-y-6">
        ...
        <div className="p-5 rounded-2xl bg-slate-800/40 border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className="text-xs font-bold px-2.5 py-0.5 rounded bg-cyan-500/20 text-cyan-300">
                {activeProvider.utilityType}
              </span>
              <span className="text-xs text-slate-400">
                Yetki Alanı: <strong className="text-white">{activeProvider.jurisdictionSide}</strong>
              </span>
            </div>
          ...
          <div className="p-3 rounded-xl bg-slate-800/80 border border-slate-700/60 text-xs text-slate-300 md:max-w-xs">
            <span className="text-[11px] text-cyan-400 font-bold uppercase block mb-0.5">Güvence Bedeli Politikası:</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-5 rounded-2xl bg-slate-800/30 border border-slate-800 flex flex-col justify-between">
              ...
              <ul className="space-y-2.5 text-xs text-slate-200">
                {activeProvider.requiredDocuments.map((doc, idx) => (
                  <li key={idx} className="flex items-start gap-2.5">
                    <span className="material-symbols-outlined text-sm text-cyan-400 flex-shrink-0 mt-0.5" aria-hidden="true">
                      check_circle
                    </span>
            ...
            <div className="mt-5 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs">
              <strong className="block text-[11px] text-rose-400 font-bold uppercase mb-0.5">
            ...
            <div>
              ...
              <div className="space-y-3">
                {activeProvider.steps.map((step) => (
                  <div key={step.stepNumber} className="p-3 rounded-xl bg-slate-800/60 border border-slate-700/60">
                    <div className="flex items-center justify-between text-xs font-semibold mb-1">
                      <span className="text-white">
                        Adım {step.stepNumber}: {step.stepTitle}
                      </span>
                      <span className="text-[11px] font-mono text-cyan-300 bg-slate-900/60 px-2 py-0.5 rounded">
                        {step.responsibleParty}
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 leading-relaxed">{step.actionRequired}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="mt-5 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs flex items-center gap-2">
      ...
      <div className="mt-8 p-4 rounded-2xl bg-slate-800/50 border border-slate-700/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-cyan-400" aria-hidden="true">support_agent</span>
          <span>
        ...
        <a
          href="/iletisim"
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold transition flex-shrink-0"
        >
