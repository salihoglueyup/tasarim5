...
const PILLARS_DATA: Record<OperationalPillarId, PillarData> = {
  rezidans: {
    ...
    icon: 'apartment',
    colorScheme: {
      badgeBg: 'bg-[var(--color-surface)] border-[var(--color-outline)]/80 text-[var(--color-primary)]',
      badgeText: 'text-[var(--color-primary)]',
      borderActive: 'border-[var(--color-primary)]',
      accentGlow: 'from-slate-500/5 via-slate-400/5 to-transparent',
      btnBg: 'bg-[var(--color-primary)] hover:opacity-90 text-[var(--color-on-primary)]',
    },
    kpis: [
    ...
    <section className="bg-[var(--color-surface)] border border-[var(--color-outline)]/80 dark:border-white/10 rounded-[3rem] p-6 sm:p-12 shadow-sm relative overflow-hidden my-16">
      ...
      <div className="text-center max-w-3xl mx-auto mb-10 relative z-10">
        ...
        <h2 className="text-2xl sm:text-4xl font-extrabold text-[var(--color-primary)] tracking-tight">
          Her Mülk Tipine Özel <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-primary)] via-slate-600 to-[var(--color-secondary)] dark:from-white dark:via-slate-200 dark:to-slate-400">Gerçek Saha Operasyonu</span>
        </h2>
        ...
        >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {current.kpis.map((kpi) => (
              <div
                key={kpi.label}
                className="p-5 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-outline)]/60 flex flex-col justify-between shadow-xs hover:border-[var(--color-outline)] hover:shadow-sm transition-all"
              >
                ...
                <span className="text-[11px] text-[var(--color-secondary)] leading-snug">{kpi.desc}</span>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {current.coreOperations.map((op) => (
              ...
              >
                ...
                <div className="pt-3 border-t border-[var(--color-outline)]/30 flex items-center justify-between text-[11px] text-[var(--color-secondary)]">
                  <span className="font-semibold text-[var(--color-secondary)]">Yasal / Teknik Standart:</span>
                  <span className="font-mono text-xs bg-[var(--color-surface-variant)] text-[var(--color-primary)] px-2.5 py-1 rounded-md border border-[var(--color-outline)]/60 font-medium">{op.standardOrLaw}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-6 rounded-2xl bg-[var(--color-surface-variant)] border border-[var(--color-outline)]/60 text-[var(--color-primary)] flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <Link
                href="/teklif-al"
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-[var(--color-primary)] hover:opacity-90 text-[var(--color-on-primary)] font-bold text-xs sm:text-sm text-center transition-all shadow-sm cursor-pointer"
              >
