      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <h2 className="text-xl sm:text-3xl font-extrabold text-[var(--color-primary)] tracking-tight">
            Sitelerde EV Şarj İstasyonu Kurulumu, <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-primary)] via-slate-600 to-[var(--color-secondary)]">KMK m.42 İzinleri & EKB Standartları</span>
          </h2>
      ...
      {activeTab === 'ev-charging' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {EV_CHARGING_OPTIONS.map((opt) => (
            <div
              key={opt.optionId}
              className="bg-[var(--color-surface-variant)]/60 border border-[var(--color-outline)]/70 hover:border-[var(--color-primary)]/50 rounded-2xl p-5 sm:p-6 flex flex-col justify-between transition shadow-2xs"
            >
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded bg-[var(--color-surface)] text-[var(--color-primary)] border border-[var(--color-outline)]/60">
                    {opt.kmkArticleRef}
                  </span>
                ...
                <div className="space-y-3 text-xs text-[var(--color-secondary)] mb-4">
                  <div className="p-3 rounded-xl bg-[var(--color-surface)] border border-[var(--color-outline)]/60">
                    <strong className="text-[var(--color-primary)] block mb-0.5 font-bold">Altyapı ve Kablolama Şartı:</strong>
                    <span>{opt.infrastructureRequirement}</span>
                  </div>

                  <div className="p-3 rounded-xl bg-[var(--color-surface)] border border-[var(--color-outline)]/60">
                    <strong className="text-[var(--color-primary)] block mb-0.5 font-bold">Faturalandırma & Sayaç Usulü:</strong>
          ...
          {ENERGY_EFFICIENCY_PILLARS.map((pillar) => (
            ...
            >
              ...
              <div className="pt-3 border-t border-[var(--color-outline)]/60 text-xs">
                <strong className="text-[11px] text-[var(--color-primary)] font-bold block mb-1.5">
