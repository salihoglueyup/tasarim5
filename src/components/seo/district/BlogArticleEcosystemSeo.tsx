...
export default function BlogArticleEcosystemSeo({
  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-outline)]/60 rounded-[2.5rem] p-6 md:p-8 mt-10 mb-8 shadow-sm">
      {/* Top Badge */}
      <div className="flex items-center justify-between gap-4 mb-6 pb-4 border-b border-[var(--color-outline)]/40">
        ...
        <span className="text-[11px] text-[var(--color-tertiary)] font-medium hidden sm:inline-block">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-6 flex flex-col justify-between p-5 rounded-2xl bg-[var(--color-surface-variant)] border border-[var(--color-outline)]/60 shadow-xs">
          <div>
            ...
            <h4 className="text-base font-extrabold text-[var(--color-primary)] mb-2">
              {recommendedSubSector.name}
            </h4>
            <p className="text-xs text-[var(--color-secondary)] leading-relaxed font-light mb-4">
        ...
        <div className="md:col-span-6 flex flex-col gap-4">
          ...
          {relevantLegislation.length > 0 && (
            <div className="p-4 rounded-2xl bg-[var(--color-surface-variant)] border border-[var(--color-outline)]/60 flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1.5">
                {relevantLegislation.map((leg) => (
                  <a
                    ...
                    className="text-xs font-semibold text-[var(--color-primary)] hover:text-blue-600 dark:hover:text-blue-400 flex items-center justify-between group py-1 border-b border-[var(--color-outline)]/30 last:border-0"
              ...
              <div className="flex flex-col gap-2">
                {relevantPrecedents.map((prec) => (
                  <div key={prec.id} className="text-xs text-slate-700 dark:text-slate-300 border-b border-slate-100 dark:border-white/5 last:border-0 pb-1.5 last:pb-0">
                    <div className="font-bold text-[var(--color-primary)] line-clamp-1">{prec.subject}</div>
                    <div className="text-[11px] text-[var(--color-secondary)] font-mono">{prec.court} · {prec.docketNumber} / {prec.decisionNumber}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {groupCompanySynergy ? (
            <div className="p-4 rounded-2xl bg-[var(--color-surface-variant)] border border-[var(--color-outline)]/60 flex items-center justify-between gap-3">
      ...
      <div className="mt-6 pt-5 border-t border-[var(--color-outline)]/40 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <Link
            href="/hesaplayici"
            className="flex-1 sm:flex-none text-center px-3.5 py-2 rounded-xl bg-[var(--color-surface-variant)] hover:bg-[var(--color-outline)]/30 border border-[var(--color-outline)]/60 text-[var(--color-primary)] text-xs font-semibold transition-all"
          >
            Bütçe & Aidat Hesapla
          </Link>
          <Link
