...
/**
 * Faz 39: LandscapeCalculator içindeki Framer Motion bağımlılıklarının kaldırılması
 * ve donanım hızlandırmalı CSS transition yapısına dönüştürülmesi.
 */
export default function LandscapeCalculator() {
  const { t } = useLanguage();
  const [greenArea, setGreenArea] = useState(2000);

  // Peyzaj & Bahçe: m2 başı 15 TL + 10,000 TL temel gübreleme/tohumlama
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-outline)]/60 rounded-[3rem] p-8 md:p-14 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-12">
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <label htmlFor="land-calc-area" className="text-sm font-bold text-[var(--color-primary)]">{t('calc_land_area')}</label>
                <span className="text-lg font-black text-slate-700 dark:text-slate-300">{greenArea.toLocaleString('tr-TR')} m²</span>
              </div>
              <input 
                id="land-calc-area"
                aria-label="Peyzaj ve Yeşil Alan (m²)"
                type="range" 
                min="500" max="25000" step="500"
                value={greenArea}
                onChange={(e) => setGreenArea(parseInt(e.target.value))}
            ...
            <div className="flex items-center gap-4 bg-slate-50 dark:bg-zinc-900 p-4 rounded-2xl border border-gray-200 dark:border-zinc-800">
               <span className="material-symbols-outlined text-slate-500 text-3xl" aria-hidden="true">nature_people</span>
               <div>
        ...
        <div className="lg:w-96 flex flex-col justify-center items-center bg-gradient-to-br from-slate-800 to-slate-950 p-10 rounded-[2.5rem] shadow-xl text-center relative border border-slate-500/20">
          ...
          {/* Faz 39: Sıfır-Jank GPU CSS Sayı Gösterimi */}
          ...
          <CalculatorLeadForm 
            serviceName="Peyzaj ve Bahçe" 
            calcDetails={{ greenArea, totalCost }}
          />
        </div>
