  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-outline)]/60 rounded-[3rem] p-10 md:p-14 shadow-sm relative overflow-hidden">
      <div className="text-center max-w-2xl mx-auto mb-12">
        <h2 className="text-3xl font-extrabold text-[var(--color-primary)]">{t('testi_dues_title')}</h2>
        <p className="text-sm text-[var(--color-secondary)] font-light mt-4">{t('testi_dues_desc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {testimonials.map((item, idx) => (
          <motion.div 
            ...
            className="bg-[var(--color-surface-variant)] p-8 rounded-3xl border border-[var(--color-outline)]/60 flex flex-col justify-between shadow-2xs"
          >
            <div>
              <span className="text-[var(--color-tertiary)] text-4xl leading-none font-serif">&ldquo;</span>
